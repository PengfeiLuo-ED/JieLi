...
class ComputationCapmVol(BaseComputation):
      computation_model_name = 'CAPM残差波动率'
...
if __name__ == '__main__':
   model = ComputationCapmVol()
   rst = model.compute_factor('E00462', 'EHSI', '20190403', 250)
   print(rst)
   #data_peer = model.compute_factor_in_peer('E00005', 'EHSI', '20190418', 250)
   #print(data_peer)
         
#table = rst
#table.loc[table['E00869'] == -np.inf,'E00869'] = 0
#matrix = np.zeros((len(table) - 250 + 1, 3))
#for i in range(0, len(matrix)):
#    model = LinearRegression()
#    x = table.iloc[i:i + 250, 1].values
#    y = table.iloc[i:i + 250, 0].values
#    print(y)
#    result = model.fit(x.reshape(-1, 1), y.reshape(-1, 1))
#    alpha = result.intercept_[0] * 250
#    beta = result.coef_[0][0]
