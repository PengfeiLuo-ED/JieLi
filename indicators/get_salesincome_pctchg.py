...
class BaseComputation(object):
      ...
      def compute_factor_in_peer(self, **kwargs):
          pass
      # endregion
class ComputeSalesIncomePctchg(BaseComputation):
      computation_model_name = '销售收入同比增长率'
      ...
      def compute_factor(self,stock_code, end_date):
          ...
          result1 = result1[['ReportType','Symbol','Currency','Turnover','YearEnd']]
          result2 = result2[['ReportType','Symbol','Currency','NetInterestIncome','YearEnd']]
          result2.columns = ['ReportType','Symbol','Currency','Turnover','YearEnd']
          result1 = pd.concat([result1,result2],axis = 0)
          result1 = result1[result1['Symbol'] == stock_code][['ReportType','Currency','Turnover','YearEnd']]
...
if __name__ == '__main__':
   compute_sales_income_pctchg = ComputeSalesIncomePctchg()
   rst = compute_sales_income_pctchg.compute_factor('E02318', '20190403')
   print(rst)
   data_peer = compute_sales_income_pctchg.compute_factor_in_peer('E02318', '20190403')
   print(data_peer)
