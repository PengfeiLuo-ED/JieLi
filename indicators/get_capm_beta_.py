# -*- coding: utf-8 -*-
"""
Created on Wed Apr 10 10:34:39 2019
...
class get_capm_beta(object):
    ...
    def get_full_data(self,code,end_date):
        Data=[]
        try:
            result = get_capm_beta.get_history_data(self,code,end_date)
        except:
        ...
        while times <= 10:
            try:
                result = get_capm_beta.get_history_data(self,code,new_end)
                Data.append(result[0])
                new_end = str(result[1]-1)    
                times += 1
            except:
    ...
    def get_factor_self(self):   
        stock_data = get_capm_beta.get_full_data(self,self.code,self.end_date)
        benchmark_data = get_capm_beta.get_full_data(self,self.benchmark,self.end_date)           
    ...
    def get_factor_peer(self,Code):
        name = get_capm_beta.get_industry(self)[1]
        print(get_capm_beta.get_industry(self)[0])
        industry_vector = np.zeros(len(name))
        for i in range(0,len(name)): 
            self.code = name[i]
            print('reading industry peers =',self.code)
            data = get_capm_beta.get_factor_self(self)       
            industry_vector[i] = data[0]
        industry_number = np.median(industry_vector)  
        self.code = Code
        history_data = get_capm_beta.get_factor_self(self)
...
if __name__ == '__main__':  
   data_self = get_capm_beta('E00700','EHSI','20190404',250).get_factor_self() 
   print('history_number =',data_self[0])
   print('history_quantile =',data_self[1])
   data_peer = get_capm_beta('E00700','EHSI','20190404',250).get_factor_peer('E00700')
