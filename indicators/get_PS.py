...
class BaseComputation(object):
      ...
      def compute_factor_in_peer(self, **kwargs):
          pass
      # endregion
class ComputePS(BaseComputation):
      computation_model_name = '市销率'
      ...
      def compute_factor(self,stock_code, end_date):
          ...
          shares = result[result['Symbol'] == stock_code][['IssuedCapital','ModifyTime']]
          shares['ModifyTime'] = shares['ModifyTime'].apply(lambda x: int(str(x)[:4] +str(x)[5:7] + str(x)[8:10])) 
          result = con.ExeQuery('SELECT * from HK_STK_FIN_Income')   
          result['Symbol'] = result['Symbol'].apply(lambda x: 'E'+x)
          share_data = result[result['Symbol'] == stock_code][['Currency','ReportType','Turnover','ModifyTime']]
          share_data.loc[share_data.Currency == 'CNY','Turnover'] = share_data.loc[share_data.Currency == 'CNY','Turnover'] / 0.85
          share_data.loc[share_data.Currency == 'JPY','Turnover'] = share_data.loc[share_data.Currency == 'JPY','Turnover'] / 5.99
          share_data.loc[share_data.Currency == 'MYR','Turnover'] = share_data.loc[share_data.Currency == 'MYR','Turnover'] / 1.62
          share_data.loc[share_data.Currency == 'SGD','Turnover'] = share_data.loc[share_data.Currency == 'SGD','Turnover'] * 4.95
          share_data.loc[share_data.Currency == 'USD','Turnover'] = share_data.loc[share_data.Currency == 'USD','Turnover'] * 7.6
          share_data.loc[share_data.ReportType == '2','Turnover'] = share_data.loc[share_data.ReportType == '2','Turnover'] * 2          
          ...
          self_data = pd.merge(self_price,share_data,on = 'ModifyTime',how = 'outer').fillna(method = 'ffill')
          self_data = pd.merge(self_data,shares,on = 'ModifyTime',how = 'outer').fillna(method = 'ffill')
...
if __name__ == '__main__':
   compute_ps = ComputePS()
   rst = compute_ps.compute_factor('E02319', '20190418')
   print(rst)
   #data_peer = compute_ps.compute_factor_in_peer('E00700', '20190418')
   #print(data_peer)
