...
class BaseComputation(object):
      ...
      def fetch_stock_data(self, *args):
          pass
      def pre_process_fetched_stock_data(self, *args):
          pass
      ...
      def fetch_industry_data(self, *args):
          pass
      def compute_factor_in_peer(self, *args):
          pass
      # endregion
class ComputationVolume(BaseComputation):
      computation_model_name = '平均成交量'
      ...
      def compute_factor_in_peer(self, stock_code, end_date, window):
          ...
          for i in range(0, len(peers)):
              ...
              computed_result = self.compute_factor(current_code, end_date, window)
              industry_vector[i] = computed_result[0]
          temp = np.sort(industry_vector)
          industry_median = np.median(temp)
          n = np.where(temp == self.compute_factor(stock_code, end_date, window)[0])
          industry_quantile = 1 - n[0][0] / len(temp)
          return [industry_median, industry_quantile]

if __name__ == '__main__':
   mean_volume_model = ComputationVolume()
   rst = mean_volume_model.compute_factor('E02319','20190403',1)
   print(rst)
   #data_peer = mean_volume_model.compute_factor_in_peer('E00700','20190403',1)
   #print(data_peer)
