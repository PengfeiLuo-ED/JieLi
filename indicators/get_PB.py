...
class BaseComputation(object):
      ...
      def compute_factor_in_peer(self, **kwargs):
          pass
      # endregion
class ComputePB(BaseComputation):
      computation_model_name = '市净率'
      ...
      def compute_factor(self,stock_code, end_date):
          ...
          result1 = result1[['Symbol','Currency','Equity','ModifyTime']]
          result2 = result2[['Symbol','Currency','Equity','ModifyTime']]
          result = pd.concat([result1,result2],axis = 0)
          share_data = result[result['Symbol'] == stock_code][['Currency','Equity','ModifyTime']]
          share_data.loc[share_data.Currency == 'CNY','Equity'] = share_data.loc[share_data.Currency == 'CNY','Equity'] / 0.85
          share_data.loc[share_data.Currency == 'JPY','Equity'] = share_data.loc[share_data.Currency == 'JPY','Equity'] / 5.99
          share_data.loc[share_data.Currency == 'MYR','Equity'] = share_data.loc[share_data.Currency == 'MYR','Equity'] / 1.62
          share_data.loc[share_data.Currency == 'SGD','Equity'] = share_data.loc[share_data.Currency == 'SGD','Equity'] * 4.95
          share_data.loc[share_data.Currency == 'USD','Equity'] = share_data.loc[share_data.Currency == 'USD','Equity'] * 7.6
          ...
          share_data['ModifyTime'] = share_data['ModifyTime'].apply(lambda x: int(str(x)[:4] +str(x)[5:7] + str(x)[8:10]))
          self_data = pd.merge(self_data,share_data,on = 'ModifyTime',how = 'outer').fillna(method = 'ffill')
...
if __name__ == '__main__':
   compute_PB = ComputePB()
   rst = compute_PB.compute_factor('E00005', '20190403')
   print(rst)
   #data_peer = compute_PB.compute_factor_in_peer('E00700', '20190418', 0.85)
   #print(data_peer)
