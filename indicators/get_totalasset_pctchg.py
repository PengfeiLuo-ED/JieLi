...
class BaseComputation(object):
      ...
      def compute_factor_in_peer(self, **kwargs):
          pass
      # endregion
class ComputeTotalAssetPctchg(BaseComputation):
      computation_model_name = '总资产同比增长率'
      ...
      def compute_factor(self,stock_code, end_date):
          ...
          result1 = result1[['ReportType','Symbol','Currency','TotalAssets','YearEnd']]
          result2 = result2[['ReportType','Symbol','Currency','TotalAssets','YearEnd']]
          result1 = pd.concat([result1,result2],axis = 0)
          result1 = result1[result1['Symbol'] == stock_code][['ReportType','Currency','TotalAssets','YearEnd']]
          result1.loc[result1.ReportType == 2,'TotalAssets'] = result1.loc[result1.ReportType == 2,'TotalAssets'] *2
          result1.loc[result1.Currency == 'CNY','TotalAssets'] = result1.loc[result1.Currency == 'CNY','TotalAssets'] / 0.85        
          result1.loc[result1.Currency == 'JPY','TotalAssets'] = result1.loc[result1.Currency == 'JPY','TotalAssets'] / 5.99
          result1.loc[result1.Currency == 'MYR','TotalAssets'] = result1.loc[result1.Currency == 'MYR','TotalAssets'] * 1.62
          result1.loc[result1.Currency == 'SGD','TotalAssets'] = result1.loc[result1.Currency == 'SGD','TotalAssets'] * 4.95
          result1.loc[result1.Currency == 'USD','TotalAssets'] = result1.loc[result1.Currency == 'USD','TotalAssets'] * 7.6
...
if __name__ == '__main__':
   compute_total_asset_pctchg = ComputeTotalAssetPctchg()
   rst = compute_total_asset_pctchg.compute_factor('E02318', '20190403')
   print(rst)
   data_peer = compute_total_asset_pctchg.compute_factor_in_peer('E02318', '20190403')
   print(data_peer)
