...
class BaseComputation(object):
      ...
      def compute_factor_in_peer(self, **kwargs):
          pass
      # endregion
class ComputeSALESCASH(BaseComputation):
      computation_model_name = '单位销售现金净流入'
      ...
      def compute_factor(self,stock_code, end_date):
          ...
          result1 = result1[['ReportType','Symbol','Currency','Turnover','ModifyTime']]
          result2 = result2[['ReportType','Symbol','Currency','NetInterestIncome','ModifyTime']]
          result2.columns = ['ReportType','Symbol','Currency','Turnover','ModifyTime']
          result1 = pd.concat([result1,result2],axis = 0)
          result1 = result1[result1['Symbol'] == stock_code][['ReportType','Currency','Turnover','ModifyTime']]
          result1.loc[result1.ReportType == 2,'Turnover'] = result1.loc[result1.ReportType == 2,'Turnover'] * 2
          result1.loc[result1.Currency == 'CNY','Turnover'] = result1.loc[result1.Currency == 'CNY','Turnover'] / 0.85        
          result1.loc[result1.Currency == 'JPY','Turnover'] = result1.loc[result1.Currency == 'JPY','Turnover'] / 5.99
          result1.loc[result1.Currency == 'MYR','Turnover'] = result1.loc[result1.Currency == 'MYR','Turnover'] * 1.62
          result1.loc[result1.Currency == 'SGD','Turnover'] = result1.loc[result1.Currency == 'SGD','Turnover'] * 4.95
          result1.loc[result1.Currency == 'USD','Turnover'] = result1.loc[result1.Currency == 'USD','Turnover'] * 7.6
          ...
          result2['Symbol'] = result2['Symbol'].apply(lambda x: 'E'+x)
          result2 = result2[['ReportType','Symbol','Currency','CEBY','NCFEY','ModifyTime']]
          result2 = result2[result2['Symbol'] == stock_code][['ReportType','Currency','CEBY','NCFEY','ModifyTime']]
          result2.loc[result2.ReportType == 2,['CEBY','NCFEY']] = result2.loc[result2.ReportType == 2,['CEBY','NCFEY']] *2
          result2.loc[result2.Currency == 'CNY',['CEBY','NCFEY']] = result2.loc[result2.Currency == 'CNY',['CEBY','NCFEY']] / 0.85        
          result2.loc[result2.Currency == 'JPY',['CEBY','NCFEY']] = result2.loc[result2.Currency == 'JPY',['CEBY','NCFEY']] / 5.99
          result2.loc[result2.Currency == 'MYR',['CEBY','NCFEY']] = result2.loc[result2.Currency == 'MYR',['CEBY','NCFEY']] * 1.62
          result2.loc[result2.Currency == 'SGD',['CEBY','NCFEY']] = result2.loc[result2.Currency == 'SGD',['CEBY','NCFEY']] * 4.95
          result2.loc[result2.Currency == 'USD',['CEBY','NCFEY']] = result2.loc[result2.Currency == 'USD',['CEBY','NCFEY']] * 7.6
          result2['ModifyTime'] = result2['ModifyTime'].apply(lambda x: int(str(x)[:4] +str(x)[5:7] + str(x)[8:10]))   
          result2 = result2.sort_values(by = 'ModifyTime')  
          result = pd.merge(result1,result2,on = 'ModifyTime',how = 'outer').fillna(method = 'ffill')      
...
if __name__ == '__main__':
   compute_salescash = ComputeSALESCASH()
   rst = compute_salescash.compute_factor('E00941', '20190423')
   print(rst)
   #data_peer = compute_salescash.compute_factor_in_peer('E00005', '20190403')
   #print(data_peer)
