...
class BaseComputation(object):
      ...
      def compute_factor_in_peer(self, **kwargs):
          pass
      # endregion
class ComptePriceTrend(BaseComputation):
    computation_model_name = '价格趋势'
    ...
    def compute_factor(self,stock_code, end_date,short_window, long_window, diff_window):
        self_close = pd.DataFrame(self.pre_process_fetched_stock_data(self.fetch_stock_data(stock_code, end_date))['close'])
        self_close['EMA_long'] = self_close['close'].ewm(adjust=False,alpha=2/(long_window+1),ignore_na=True).mean()
        self_close['EMA_short'] = self_close['close'].ewm(adjust=False,alpha=2/(short_window+1),ignore_na=True).mean()
    ...
    def compute_factor_in_peer(self, stock_code, end_date,short_window, long_window, diff_window):
        ...
        for i in range(0,len(peers)):             
            ...
            computed_result = self.compute_factor(current_code,end_date,short_window, long_window, diff_window)
        ...
        history_data = self.compute_factor(stock_code, end_date,short_window, long_window, diff_window)
...
if __name__ == '__main__':
    compute_ret_vol = ComptePriceTrend()
    rst = compute_ret_vol.compute_factor('E00700','20190403',12,26,9)
    print(rst)
    #data_peer = compute_ret_vol.compute_factor_in_peer('E00700','20190403',12,26,9)
