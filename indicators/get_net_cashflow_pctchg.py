...
class BaseComputation(object):
      ...
      def compute_factor_in_peer(self, **kwargs):
          pass
      # endregion
class ComputeNetCashFlowPctchg(BaseComputation):
      computation_model_name = '净现金流同比增长率'
      ...
      def compute_factor(self,stock_code, end_date):
          ...
          result2 = result2[['Symbol','Currency','CEBY','NCFEY','YearEnd']]
          result2 = result2[result2['Symbol'] == stock_code][['Currency','CEBY','NCFEY','YearEnd']]
          ...
          del result2['Currency']
          result2['result'] = (result2['NCFEY'] - result2['CEBY'])  
          result2 = result2.sort_values(by = 'YearEnd')
          ...
          result2 = pd.concat([re6,re12],axis = 0).sort_values(by = 'YearEnd').dropna()  
...
if __name__ == '__main__':
   compute_net_cashflow_pctchg = ComputeNetCashFlowPctchg()
   rst = compute_net_cashflow_pctchg.compute_factor('E02318', '20190403')
   print(rst)    
   data_peer = compute_net_cashflow_pctchg.compute_factor_in_peer('E02318', '20190403')
   print(data_peer)
