...
class BaseComputation(object):
      ...
      def compute_factor_in_peer(self, **kwargs):
          pass
      # endregion
class ComputePE(BaseComputation):
      computation_model_name = '市盈率'
      ...
      def compute_factor(self,stock_code, end_date):
          ...
          result = con.ExeQuery('SELECT * from HK_STK_EarningsSummary')
          ...
          share_data = result[result['Symbol'] == stock_code][['EPS_HKD','ReportType','ModifyTime',]]          
          share_data['ModifyTime'] = share_data['ModifyTime'].apply(lambda x: int(str(x)[:4] + str(x)[5:7] + str(x)[8:10]))
          share_data.loc[share_data.ReportType=='2','EPS_HKD'] = share_data.loc[share_data.ReportType=='2','EPS_HKD'] * 1.8           
...
if __name__ == '__main__':
   compute_pe = ComputePE()
   rst = compute_pe.compute_factor('E02319', '20190403')
   print(rst)
   #data_peer = compute_pe.compute_factor_in_peer('E00005', '20190403')
   #print(data_peer)
