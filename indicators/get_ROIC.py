# -*- coding: utf-8 -*-
"""
Created on Mon Apr 29 10:21:35 2019
...
class BaseComputation(object):
      ...
      def compute_factor_in_peer(self, **kwargs):
          pass
      # endregion
class ComputeROIC(BaseComputation):
      computation_model_name = 'ROIC'
      ...
      def compute_factor(self, stock_code, end_date):
          ...
          result1 = result1[['ReportType','Symbol','ReturnOnCapitalEmploy','YearEnd']]
          result2 = result2[['ReportType','Symbol','ReturnOnLoans','YearEnd']]      
          result2.columns = ['ReportType','Symbol','ReturnOnCapitalEmploy','YearEnd']
          result1 = pd.concat([result1,result2],axis = 0)
          del result2
          result1 = result1[result1['Symbol'] == stock_code][['ReportType','ReturnOnCapitalEmploy','YearEnd']]
          result1 = result1.sort_values(by = 'YearEnd')
          result1.loc[result1.ReportType=='2','ReturnOnCapitalEmploy'] = result1.loc[result1.ReportType=='2','ReturnOnCapitalEmploy'] * 2           
...
if __name__ == '__main__':
   compute_roic = ComputeROIC()
   rst = compute_roic.compute_factor('E02319', '20190403')
   print(rst)
    
   data_peer = compute_roic.compute_factor_in_peer('E02318', '20190403')
   print(data_peer)
